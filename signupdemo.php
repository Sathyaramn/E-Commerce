...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  if(empty($_POST["storeName"])){
    $snameErr="Store name is required";
  }
  ...
  if(!preg_match($mobilePattern,$adminMobile)){
...
";
  ...
  //to check whether the admin already exists
  $emailQuery="select * from adminPersonalDetails where admin_email='$adminEmail'";
  $mobileQuery="select mobile_number from adminPersonalDetails where mobile_number='$adminMobile'";
  ...
  if(mysqli_num_rows($checkEmailSql)>0||mysqli_num_rows($checkMobileSql)>0){
    $adminExistsError="Admin already exists";
  }
  else{
...
<div class="container-1">
  <form  class="form-1" id="form-1" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" enctype="multipart/form-data">
    ...
    <span style="color:red"><?php echo $adminExistsError; ?></span>
    ...
    <span><?php echo $lnameErrMsg; ?></span>
        <br>
    <label>Store Name:</label><input type="text" name="storeName"></input>
